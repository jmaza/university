'use strict';

// Load modules

const Path = require('path');


// Declare internals

const internals = {
    rootPath: Path.resolve(__dirname, '../'),
    viewsPath: Path.resolve(__dirname, '../views')
};


exports.register = function (server, options, next) {

    // Code inside the callback function of server.dependency will only be
    // executed after vision and inert has been registered.  It's triggered by
    // server.start, and runs before the starting of the server.  It's done because
    // resources that utilize views depend on vision and inert.
    // Earlier versions of hapi included vision and inert out of hapi. Now, these must be included seperately.

    server.dependency(['AuthCookie','inert', 'vision'], internals.after);

    return next();
};

exports.register.attributes = {
    name: 'Home'
};


internals.after = function (server, next) {

    server.views({
        engines: {
            html: require('handlebars')
        },
        path: '../views',
        partialsPath: '../views/partials',
        relativeTo: __dirname
    });

    // Routing for static files
    server.route([

        // Images

        {
            method: 'GET',
            path: '/images/{assetpath*}',
            config: {
                auth: { strategy: 'session', mode: 'try' },
                plugins: { 'hapi-auth-cookie': { redirectTo: false } },
                handler: {
                    directory: {
                        path: './assets/images'
                    }
                }
            }
        },

        // Scripts

        {
            method: 'GET',
            path: '/scripts/{assetpath*}',
            config: {
                auth: { strategy: 'session', mode: 'try' },
                plugins: { 'hapi-auth-cookie': { redirectTo: false } },
                handler: {
                    directory: {
                        path: './assets/scripts'
                    }
                }
            }
        },

        // Styles

        {
            method: 'GET',
            path: '/styles/{assetpath*}',
            config: {
                auth: { strategy: 'session', mode: 'try' },
                plugins: { 'hapi-auth-cookie': { redirectTo: false } },
                handler: {
                    directory: {
                        path: './assets/styles'
                    }
                }
            }
        }
    ]);

    // Home related routing
    server.route([

        // Login route

        {
            method: 'GET',
            path: '/login',
            config: {
                description: 'Returns a login form',
                auth: { strategy: 'session', mode: 'try' },
                plugins: { 'hapi-auth-cookie': { redirectTo: false } },
                handler: {
                    view: {
                        template: 'login'
                    }
                }
            }
        },

        // Home route

        {
            method: 'GET',
            path: '/home',
            config: {
                description: 'Returns the home page',
                auth: { strategy: 'session', mode: 'try' },
                plugins: { 'hapi-auth-cookie': { redirectTo: false } },
                handler: (request, reply) => {
                    let userData = null;
                    
                    if (request.auth.isAuthenticated) {
                        userData = { username: request.auth.credentials.username, email:  request.auth.credentials.username };
                    }

                    return reply.view('home', { user: userData });
                }
            }
        },
        {
            method: 'GET',
            path: '/account',
            config: {
                description: 'Returns the account page',
                auth: { strategy: 'session', mode: 'try', scope: ['user', 'admin'] },
                plugins: { 'hapi-auth-cookie': { redirectTo: false } },
                handler: (request, reply) => {
                    let userData = null;
                    
                    if (request.auth.isAuthenticated) {
                        userData = { username: request.auth.credentials.username, email:  request.auth.credentials.username };
                    }

                    return reply.view('account', { user: userData, admin: request.auth.credentials.scope[0] === 'admin' });
                }
            }
        },
        {
            method: 'GET',
            path: '/admin',
            config: {
                description: 'Returns the admin page',
                auth: { strategy: 'session', mode: 'try', scope: ['admin'] },
                plugins: { 'hapi-auth-cookie': { redirectTo: false } },
                handler: (request, reply) => {
                    let userData = null;
                    
                    if (request.auth.isAuthenticated) {
                        userData = { username: request.auth.credentials.username, email:  request.auth.credentials.username };
                    }

                    return reply.view('admin', { user: userData, admin: request.auth.credentials.scope[0] === 'admin' });
                }
            }
        }
    ]);

    return next();
};
